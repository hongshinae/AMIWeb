<template>
	<b-modal id="dcuPing" @shown="shown" @show="show" @hide="hide" @hidden="hidden" @ok="ok" @cancel="cancel" no-close-on-backdrop>
		...
		<div class="modal-content-wrap">
			<b-overlay id="overlay-background" :show="isLoading" variant="light" opacity="0.85" blur="2px" rounded="sm">
				<div class="btn-wrap">
					<ul>
						<li>
							<b-button variant="light" @click="sendDcuReboot">DCU 재부팅</b-button>
							<small>{{ dcuId }} ({{ dcuIp }})</small>
						</li>
					</ul>
				</div>
...
export default {
	...
	methods: {
		...
		async sendDcuPing() {
			try {
				this.isLoading = true;
				const response = await Status.ping({ ip: this.dcuIp });
